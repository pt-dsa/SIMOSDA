...
export default function Inventaris() {
  ...
  const handleBulkDelete = () => {
    if (confirm(`Apakah anda yakin ingin menghapus ${selectedRows.length} data inventaris secara massal?`)) {
  ...
  const handleBulkUpdateStatus = (newStatus: string) => {
    if (confirm(`Apakah anda yakin ingin mengubah status ${selectedRows.length} item menjadi ${newStatus}?`)) {
      ...
      setData(prev => prev.map(item => idsToUpdate.has(item.asset_id || (item as any).id) ? { ...item, kondisi: newStatus } as Inventory : item));
      setSelectedRows([]);
