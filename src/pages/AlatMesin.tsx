...
export default function AlatMesin() {
  ...
  const handleDelete = (id: string) => {
    if (confirm("Apakah anda yakin ingin menghapus data alat & mesin ini?")) {
      setData(prev => prev.filter(item => item.asset_id !== id && (item as any).id !== id));
      setSelectedRows(prev => prev.filter(item => item.asset_id !== id && (item as any).id !== id));
      toast.success("Data Dihapus", "Data alat & mesin berhasil dihapus.");
    }
  };

  const handleBulkDelete = () => {
    if (confirm(`Apakah anda yakin ingin menghapus ${selectedRows.length} data secara massal?`)) {
      const idsToDelete = new Set(selectedRows.map(r => r.asset_id || (r as any).id));
      setData(prev => prev.filter(item => !idsToDelete.has(item.asset_id || (item as any).id)));
      setSelectedRows([]);
  ...
  const handleBulkUpdateStatus = (newStatus: string) => {
    if (confirm(`Apakah anda yakin ingin mengubah status ${selectedRows.length} item menjadi ${newStatus}?`)) {
      const idsToUpdate = new Set(selectedRows.map(r => r.asset_id || (r as any).id));
      setData(prev => prev.map(item => idsToUpdate.has(item.asset_id || (item as any).id) ? { ...item, kondisi: newStatus } : item));
      setSelectedRows([]);
  ...
  const handleSave = (e: React.FormEvent) => {
    e.preventDefault();
    if (formData.asset_id || (formData as any).id) {
      setData(prev => prev.map(item => (item.asset_id === formData.asset_id || (item as any).id === (formData as any).id) ? { ...item, ...formData } as Equipment : item));
      toast.success("Data Disimpan", "Perubahan data alat & mesin berhasil disimpan.");
    } else {
      const newItem = {
        ...formData,
        asset_id: `EQP${Date.now()}`,
        id: `EQP${Date.now()}`
      } as Equipment;
