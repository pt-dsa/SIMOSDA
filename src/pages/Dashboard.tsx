...
import {
  Users, UserCheck, AlertTriangle, Clock, Calendar,
...
const itemVars = { hidden: { opacity: 0, y: 18 }, show: { opacity: 1, y: 0, transition: { type: "spring", stiffness: 280, damping: 22 } } };
...
// ---------------------------------------------------------------------------
function AlertCard({ title, count, subtitle, colorScheme }: {
  title: string; count: number; subtitle: string; colorScheme: "yellow" | "blue" | "red";
}) {
  const P = {
    ...
    <div className="space-y-2.5">
      {data.map((item, idx) => (
        <div key={item.name} className="flex items-center gap-3">
          <span className="text-xs font-medium text-gray-600 dark:text-gray-400 w-16 shrink-0 text-right truncate">{item.name}</span>
...
// ---------------------------------------------------------------------------
// Setup Guide — muncul ketika sheet "pegawai" belum dikonfigurasi
// ---------------------------------------------------------------------------
function PegawaiSetupGuide({ errorMsg, onRetry }: { errorMsg: string; onRetry: () => void }) {
  const SPREADSHEET_URL = `https://docs.google.com/spreadsheets/d/19EllcHpSDAANnoXcTCYI8LIR9TE7b_e_Cst0KMgveO0`;
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          <h3 className="font-bold text-amber-800 dark:text-amber-400 text-lg mb-1">
            Sheet "pegawai" Belum Dikonfigurasi
          </h3>
          <p className="text-sm text-amber-700 dark:text-amber-500 mb-4">
            Data kepegawaian belum tersedia karena sheet <code className="bg-amber-100 dark:bg-amber-800 px-1 py-0.5 rounded text-xs font-mono">pegawai</code> belum ditambahkan ke Google Sheets.
          </p>
          ...
          <div className="bg-white/60 dark:bg-gray-900/40 rounded-xl p-4 mb-4">
            ...
            <ol className="space-y-2 text-sm text-gray-600 dark:text-gray-400 list-none">
              {[
                `Buka Google Sheets SIKANDA`,
                `Klik tombol "+" di bagian bawah untuk menambah sheet baru`,
                `Beri nama persis: pegawai (huruf kecil semua)`,
                `Tambahkan header di baris 1 (lihat daftar kolom di bawah)`,
                `Salin 45 data pegawai dari file Excel ke sheet ini`,
          ...
          <div className="bg-white/60 dark:bg-gray-900/40 rounded-xl p-4 mb-4">
            <p className="text-xs font-bold text-gray-600 dark:text-gray-400 mb-2 uppercase tracking-wide">
              Header kolom yang diperlukan (baris 1):
            </p>
            <div className="flex flex-wrap gap-1.5">
              {["NO","NAMA PEGAWAI","NIP","GOLONGAN","TERHITUNG MULAI TANGGAL (GOLONGAN)",
                "JABATAN","TERHITUNG MULAI TANGGAL (JABATAN)","MASA KERJA (TAHUN)",
                "MASA KERJA (BULAN)","RIWAYAT DIKLAT","TAHUN DIKLAT","PENDIDIKAN (JURUSAN)",
                "TAHUN LULUS","TINGKAT","UNIVERSITAS","TANGGAL LAHIR","USIA","STATUS",
                "CATATAN MUTASI (MASUK)","CATATAN MUTASI (KELUAR)","KONTAK","KETERANGAN","FOTO"
              ].map((col) => (
                <code key={col} className="text-[10px] bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 rounded font-mono text-gray-700 dark:text-gray-300">
                  {col}
                </code>
              ))}
            </div>
      ...
      if (msg.includes("pegawai") || msg.includes("tidak ditemukan")) {
        ...
        try {
          ...
          setMetrics({
            ...
            peringatanKGB: 0, peringatanPangkat: 0, peringatanPensiun: 0,
                      ...
                      <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 mt-3 w-full">
                        {metrics.distribusiGolongan.map((item, i) => (
                          <div key={item.name} className="flex items-center gap-1.5">
                            ...
                            <span className="text-xs font-bold text-gray-900 dark:text-gray-100 ml-auto">{item.value}</span>
                          </div>
                        ))}
                      </div>
                    </>
                      ...
                      <div className="grid grid-cols-2 gap-x-4 gap-y-2 w-full">
                        {[{ name: "Kendaraan", val: metrics.totalKendaraan, color: "#0B57D0" }, { name: "Alat & Mesin", val: metrics.totalAlatMesin, color: "#34A853" }, { name: "Inventaris", val: metrics.totalInventaris, color: "#FBBC04" }].map((item) => (
                          <div key={item.name} className="flex flex-col">
                            ...
                            <span className="text-sm font-bold text-gray-900 dark:text-gray-100 pl-4">{formatNumber(item.val)}</span>
                          </div>
                        ))}
                      </div>
                    </>
